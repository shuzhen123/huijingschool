...
import common.date.DateStyle;
import common.date.DateUtility;

 
/**
 * * 利用反射通过bean之间的对比对属性进行修改/更新等 * * @author only29
 */
public class BeanRefUtility {
	...
	/**
	 * 从source(old 实体)对应属性更新到target(从数据库获取的实体) 并判断新老对象属性值是否全是一致(即是否需要更新到数据库)
	 * 
	 * @param source
	 * @param target
	...
	public static String getFieldValue(Object bean, Field field) throws Exception {
		...
		if (checkGetMethod(methods, fieldGetName)) {
			...
			if ("Date".equals(fieldType)) {
				result = DateUtility.DateToString((Date) fieldVal,DateStyle.YYYY_MM_DD_HH_MM_SS);
			} else {
				if (null != fieldVal) {
		...
		if (checkSetMethod(methods, fieldSetName)) {
			...
			if (validNullAndEmptyString(value)) {
				...
				if ("String".equals(fieldType)) {
					...
					Date temp = DateUtility.StringToDate(value);
