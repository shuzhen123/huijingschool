...
import dianfan.pay.unionpay.sdk.LogUtil;
import dianfan.pay.unionpay.sdk.SDKConfig;
...
public class UnionPayCore {
	 ...
	 * @throws Exception
	 * @throws:
	 * @time: 2018年5月26日 上午11:22:29
	 */
	public String unionPay(String merId, String txnAmt) throws Exception {
		// 加载sdkProperties
		...
		requestData.put("channelType", SDKConfig.getConfig().getChannelType()); // 渠道类型，这个字段区分B2C网关支付和手机wap支付；07：PC,平板
																				// 08：手机
		...
		requestData.put("payTimeout", new SimpleDateFormat("yyyyMMddHHmmss")
				.format(new Date().getTime() + SDKConfig.getConfig().getPayTimeout()));
