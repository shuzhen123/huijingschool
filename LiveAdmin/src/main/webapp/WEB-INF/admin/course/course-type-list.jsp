	...
	<div class="modal-dialog">
		<div class="modal-content radius">
			<div class="modal-header">
				<h3 class="modal-title">添加课程类型</h3>
				...
				<form class="form form-horizontal" id="form" >
					<div class="row cl">
						...
						<div class="formControls col-xs-8 col-sm-7">
							<input type="text" class="input-text" placeholder="请填写类型名称" name="coursetypename">
						</div>
					</div>
				</form>
			</div>
...
<div id="editModal" class="modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content radius">
			<div class="modal-header">
				<h3 class="modal-title">编辑课程类型</h3>
				<a class="close" data-dismiss="modal" aria-hidden="true" href="javascript:void();">×</a>
			</div>
			<div class="modal-body">
...
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页  <span class="c-gray en">&gt;</span> 课程类型管理 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
...
$(function(){
        ...
        columns: [
            ...
            {"data": 'createtime'},
            {
                ...
                "render": function (base_data, type, full) { 
                     var str = 
	                    '<a title="编辑" href="javascript:;" class="ml-5 course-type-edit" style="text-decoration:none">' +
	                    '	<i class="Hui-iconfont" style="color: #3bb4f2;">&#xe6df;</i>' +
	                    '</a> ' +
	                    '<a title="删除" href="javascript:;" class="ml-5 course-type-del" style="text-decoration:none">' +
    ...
    //删除
    $("body").on("click", "a.course-type-del", function() {
    	var sData = api_table.fnGetData( $(this).parents("tr") );
    	layer.confirm('确定删除此类型？删除后此类型下的所有课程也将被删除！', {icon: 3, title:'提示'}, function(index){
    		betchdel('delCardByid', [sData.id], jq_table);
    	});
    });
    ...
    $(".batch-del").click(function() {
    	...
    	if(cblist.length > 0){
			...
			layer.confirm('确定批量删除类型？删除后此类型下的所有课程也将被删除！', {icon: 3, title:'批量删除'}, function(index){
				betchdel('delCardByid', ids, jq_table);
	    	});
    	}
    ...
    $("#myModal .confirm").click(function() {
    	var name = $.trim($("#myModal input[name='coursetypename']").val());
    	if(name == ''){
    		layer.msg('请填写类型名称！',{icon: 7,time:2000}); return false;
    	}
    	//检测通过
    	$.ajax({
			type: 'POST',
    ...
    //修改类型
    $("body").on("click", "a.course-type-edit", function() {
    	var sData = api_table.fnGetData( $(this).parents("tr") );
    	$("#editModal input[name='coursetypename']").val(sData.coursetypename);
    	$("#editModal input[name='id']").val(sData.id);
    	$("#editModal").modal('show');
    })
    $("#editModal .confirm").click(function() {
    	var id = $("#editModal input[name='id']").val(), name = $.trim($("#editModal input[name='coursetypename']").val());
    	if(name == ''){
    		layer.msg('请填写类型名称！',{icon: 7,time:2000}); return false;
    	}
    	//检测通过
    	$.ajax({
			type: 'POST',
			url: 'editcoursetype',
			data: {id: id, coursetypename: name},
			success: function(data){
				if(data.code == 200){
