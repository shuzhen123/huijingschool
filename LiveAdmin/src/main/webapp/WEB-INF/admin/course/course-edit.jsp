...
<article class="page-container">
	<form action="editcourse" method="post" enctype="multipart/form-data" class="form form-horizontal" id="form">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<span class="select-box" style="width: auto;">
					<select class="select" name="coursekind" size="1">
						...
						<%-- <option value="1" <c:if test="${course.coursekind==1}">selected</c:if> >免费直播课程</option> 
						<option value="2" <c:if test="${course.coursekind==2}">selected</c:if> >vip实战直播课程</option> --%>
		...
		<c:if test="${course.auth != 2}">
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-3">课程审核：</label>
				<div class="formControls col-xs-8 col-sm-9">
					<span class="select-box" style="width:auto;">
						<select class="select" name="auth" size="1">
							<option value="2" <c:if test="${course.auth==2}">selected</c:if> >通过</option> 
							<option value="3" <c:if test="${course.auth==3}">selected</c:if> >不通过</option>
						</select>
					</span>
				</div>
			</div>
		...
		<div class="recommend <c:if test="${course.auth==3}">hidden</c:if> ">
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-9">
					<span class="select-box" style="width:auto;">
						<select class="select" name="recommendflag" size="1">
							<option value="0" <c:if test="${course.recommendflag==0}">selected</c:if> >否</option> 
							<option value="1" <c:if test="${course.recommendflag==1}">selected</c:if> >是</option>
						</select>
					</span>
				</div>
			</div>
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-9">
					<span class="select-box" style="width:auto;">
						<select class="select" name="bannerflag" size="1">
							<option value="0" <c:if test="${course.bannerflag==0}">selected</c:if> >否</option> 
							<option value="1" <c:if test="${course.bannerflag==1}">selected</c:if> >是</option>
						</select>
					</span>
				</div>
			</div>
...
$(function(){
	...
	//课程推荐状态切换
	$('select[name="recommendflag"]').change(function() {
		var val = $(this).val();
		if(val == 1){
			//是
			$("div.recommendindex").removeClass("hidden");
		}else{
			//否
			$("div.recommendindex").addClass("hidden");
		}
	})
