...
<article class="page-container">
	<form action="${pageContext.request.contextPath}/tea/editlivecourse" method="post" enctype="multipart/form-data" class="form form-horizontal" id="form">
		...
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3">直播类型：</label>
			<div class="formControls col-xs-8 col-sm-9">
			...
			<div class="formControls col-xs-8 col-sm-9">
				...
				<span class="select-box" style="width: auto;">
					<select class="select" name="auth" size="1">
						<option value="1" <c:if test="${course.auth == 1 }">selected</c:if> >待审核</option>
						<option value="2" <c:if test="${course.auth == 2 }">selected</c:if> >通过</option>
						<option value="2" <c:if test="${course.auth == 3 }">selected</c:if> >不通过</option>
					</select>
				</span>
			</div>
		</div>
