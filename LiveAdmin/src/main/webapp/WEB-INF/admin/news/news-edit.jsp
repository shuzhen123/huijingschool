...
<article class="page-container">
	<form action="editnews.do" method="post" enctype="multipart/form-data" class="form form-horizontal" id="form">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${info.infomationtitle }" placeholder="请输入资讯标题" name="infomationtitle">
			</div>
		</div>
		<div class="row cl">
				...
				<span class="select-box" style="width: auto;">
					<select name=informationmodelid class="select">
						<c:forEach var="item" items="${model}">
							<option value="${item.informationid }" <c:if test="${info.informationmodelid == item.informationid }">selected</c:if> >${item.newskindname }</option>
						</c:forEach>
					</select>
				</span> 
			</div>
			...
			<div class="formControls col-xs-8 col-sm-9">
				...
				<span class="select-box" style="width: auto;">
					<select class="select" id="agent">
						...
						<c:forEach var="item" items="${agent}">
							<option value="${item.id }" <c:if test="${agentid == item.id }">selected</c:if> >${item.name }</option>
						</c:forEach>
					</select>
				</span> 
				讲师：
				<span class="select-box" style="width: auto;">
					<select name="userid" class="select">
						...
						<c:forEach var="item" items="${teas}">
							<option value="${item.id }" <c:if test="${info.userid == item.id }">selected</c:if> >${item.name }</option>
						</c:forEach>
					</select>
				</span> 
			</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${info.singname }" placeholder="请输入作者姓名" name="singname">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">标签：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${info.tag }" placeholder="请输入标签" name="tag">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">描述：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<textarea name="descrption" class="textarea" placeholder="请输入描述信息" >${info.descrption }</textarea>
			</div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<c:if test="${info.picurl != null}">
					<div><img src="${info.picurl }" style="width: 150px;margin-bottom: 10px;"></div>
				</c:if>
				<div>
