...
<![endif]-->
<style type="text/css">
	table#datatable tr.order-info td.order-info{
		text-align: left;
	}
</style>
<title>课程订单列表</title>
...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页  <span class="c-gray en">&gt;</span> 课程订单列表<a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
...
$(function(){
	...
	var jq_table = $("#datatable").DataTable({
        ...
        columns: [
            ...
            {"data": 'money'},
            {
                ...
                "mRender": function (data, type, full, meta) {
                	if(full.paystatus == 0){
	                	...
	                	return '<span class="label label-default radius">已取消</span>';
                	}
                }
            },
            {"data": 'createtime'},
            ...
            },
            {
                ...
                "mRender": function (data, type, full, meta) {
                	if(full.paystatus == 1){
                		...
                		else if(full.paytype == 2) return '微信';
                		else if(full.paytype == 3) return '线下打款';
                		else return '未知';
                	}else{
                		return '-'
                	}
            ...
            },
            {
                ...
                "class": "course",
                "mRender": function (data, type, full, meta) {
	            	return '<button class="btn btn-primary size-MINI courses" type="button">查看课程</button>';
                }
            },
            {"data": "realname"},
            {
                ...
                "mRender": function (data, type, full, meta) {
                	if(full.paystatus == 0){
	                	var str = '<a title="编辑" href="javascript:;" class="ml-5 order-edit" style="text-decoration:none">' +
	...
	//课程订单页
	$("body").on('click', 'a.order-edit', function() {
		var sData = api_table.fnGetData( $(this).parents("tr") );
    	layer_show('课程订单编辑', "corsetordereditpage?orderid=" +sData.orderno + "&userid="+sData.userid,'','510');
	})
	//查看课程列表
	$("body").on('click', 'button.courses', function() {
        ...
        if (api_table.fnIsOpen(obj)) {
            ...
            var course_arr = sData.coursename.split(',');
            var str = '';
            $.each(course_arr, function(i, v) {
            	str += '<span style="color: red;">《' + v+ '》</span>    '
			})
            api_table.fnOpen(obj, str, "info_row order-info");
        }
	})
    //外围刷新
