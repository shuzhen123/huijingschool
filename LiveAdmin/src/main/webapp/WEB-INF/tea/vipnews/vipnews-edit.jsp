...
<article class="page-container">
	<form action="editvipnews" method="post" enctype="multipart/form-data" class="form form-horizontal" id="form">
		<input type="text" class="hidden" value="${info.articalid }" name="articalid">
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">资讯标题：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${info.articaltitle }" placeholder="请输入资讯标题" name="articaltitle">
			</div>
		</div>
		<div class="row cl">
				...
				<span class="select-box" style="width:auto;">
					<select class="select" name="kind" size="1">
						<c:forEach var="item" items="${kind }">
							<option value="${item.id }" <c:if test="${info.kind == item.id }">selected</c:if> >${item.levelname }（${item.money }元）</option> 
						</c:forEach>
					</select>
				</span>
			</div>
			...
			<div class="formControls col-xs-8 col-sm-9">
				<span class="select-box" style="width: auto;">
					<select name="showflag" class="select">
						<option value="1" <c:if test="${info.showflag==1}">selected</c:if> >显示</option>
						<option value="0" <c:if test="${info.showflag==0}">selected</c:if> >不显示</option>
					</select>
				</span> 
			</div>
		</div>
