			...
			<c:forEach var="item" items="${videos }">
				...
				data-intro="${item.videointroduce }" data-content="${item.videocontent }" data-video="${item.videourl }" data-enabled="${item.enableflag }" >
					<div class="portfoliobox">
						<input class="checkbox <c:if test="${item.entkbn == 1 }">hidden</c:if>" type="checkbox" >
						<div class="picbox">
							<c:if test="${item.entkbn == 1 }">
								<img src="${item.videoppicurl }" <c:if test="${item.enableflag == 2 }">style="filter: alpha(opacity=50);opacity: .5;"</c:if>  >
							</c:if>
							<c:if test="${item.entkbn == 0 }">
						...
						<c:if test="${item.entkbn == 1 }">
							<div class="transcodeing" style="color: red;">视频转码中...</div>
						</c:if>
						<div class="textbox">
							${item.videoname }
							<a class="r edit btn btn-success-outline radius size-MINI edit-video-btn <c:if test="${item.entkbn == 1 }">hidden</c:if>"  title="编辑视频"><i class="Hui-iconfont">&#xe6df;</i></a>
						</div>
					</div>
				</li>
			</c:forEach>
	...
	<div class="lightbox" style="display: block; top: 100px; left: 0px;">
		<div class="lb-outerContainer" style="width: 50%; height: 100%;">
			<div class="lb-container">
				<video id="my-player" class="video-js vjs-default-skin vjs-big-play-centered" controls preload="auto" width='100%' height='100%'>
...
<!--请在下方写此页面业务相关的脚本-->
<script type="text/javascript" src="${pageContext.request.contextPath}/lib/md5.js"></script>
...
<%-- <script type="text/javascript" src="${pageContext.request.contextPath}/lib/neplayer/nep.min.js"></script> --%>
<script type="text/javascript">

$(function(){
	var timer = setInterval(function(){
	    var obj = $("li .transcodeing");
	    if(obj.length > 0){
	    	var ids = [];
	    	$.each(obj, function(k, v) {
	    		ids.push($(v).parents('li').attr('data-id'));
			})
			$.ajax({
	            url: '${pageContext.request.contextPath}/tea/videotcsttatus',
	            type: "POST",
	            data: {vids: ids},
	            success: function (result) {
	            	if(result.code == 200){
	            		$.each(result.data, function(k, v) {
	        	    		if(v.entkbn == 0){
	        	    			//视频转码完成
	        	    			$('li[data-id="'+ v.videoid +'"]').find('.transcodeing').remove();
	        	    			$('li[data-id="'+ v.videoid +'"]').find('.checkbox').removeClass('hidden');
	        	    			$('li[data-id="'+ v.videoid +'"]').find('.edit-video-btn').removeClass('hidden');
	        	    			$('li[data-id="'+ v.videoid +'"]').attr('data-video', v.videourl);
	        	    			$('li[data-id="'+ v.videoid +'"]').find('.picbox img').wrap('<a href="javascript:;" title="点击播放视频" ></a>');
	        	    		}
	        			})
	            	}
	            }
	...
	var opt = {
	    ...
	    onProgress: function(curFile){//上传进度回调处理函数
	    	$('li[data-key="'+ curFile.fileKey +'"]').find('.pg-bar .r').html(curFile.progress+"%");
	    	$('li[data-key="'+ curFile.fileKey +'"]').find('.pg-bar .sr-only').css('width', curFile.progress+"%");
	    },
	...
	$(".add-video-model").click(function() {
		...
		if(addservice == undefined){
			...
			opt.onUploaded = function(curFile){//单文件上传成功的回调函数
		    	...
		    	//更改视频状态
		    	$.ajax({
		            url: '${pageContext.request.contextPath}/tea/updatevideourl',
		            type: "POST",
		            data: {"videoid": $('li[data-key="'+ curFile.fileKey +'"]').attr('data-id'), "filename": curFile.objectName},
		            dataType: "json",
		            success: function (result) {
		            	if(result.code == 200){
		            		$('li[data-id="'+ result.data +'"]').find('.textbox').before('<div class="transcodeing" style="color: red;">视频转码中...</div>');
		            	}else{
		            		layer.msg('视频数据更新出错，请重新上传！',{icon: 5,time:2000});
		            		$('li[data-id="'+ result.data +'"]').remove();
		            	}
		            	//启用添加按钮
		        		$(".add-video-model").removeClass('disabled');
		            }
	...
	$("#confirm").click(function() {
		...
		$.ajax({
            url: '${pageContext.request.contextPath}/tea/addvideo',
            type: "POST",
            data: data,
            dataType: "json",
            processData: false,
            ...
            success: function (result) {
            	if(result.code == 200){
            		...
            		var str = 
            			...
            			'      </div>' +
            			'    </div>' +
            			'    <div class="textbox">'+ data.videoname +
            			'		<a class="r edit btn btn-success-outline radius size-MINI edit-video-btn hidden"  title="编辑视频"><i class="Hui-iconfont">&#xe6df;</i></a>' +
            			'	 </div>' +
            			'  </div>' +
            			'</li>';
            		$("ul").append(str);
		...
		if(editservice == undefined){
			...
			opt.onUploaded = function(curFile){//单文件上传成功的回调函数
		    	...
		    	$.ajax({
		            ...
		            success: function (result) {
		            	if(result.code == 200){
		            		$('li[data-id="'+ result.data +'"]').find('.picbox').html($('li[data-id="'+ result.data +'"]').find('.picbox img'));
		            		$('li[data-id="'+ result.data +'"]').find('.textbox').before('<div class="transcodeing" style="color: red;">视频转码中...</div>');
		            	}else{
		            		layer.msg('视频数据更新出错，请重新上传！',{icon: 5,time:2000});
		            	}
	...
	$("#reconfirm").click(function() {
        ...
        var data =new FormData(form);
		$.ajax({
            url: '${pageContext.request.contextPath}/tea/editvideo',
            type: "POST",
            data: data,
            dataType: "json",
            processData: false,
