	...
	<div class="modal-dialog">
		<div class="modal-content radius">
			<div class="modal-header">
				<h3 class="modal-title">添加新部门</h3>
				<a class="close" data-dismiss="modal" aria-hidden="true" href="javascript:void();">×</a>
			</div>
			<div class="modal-body">
				<form class="form form-horizontal" id="form" >
					<div class="row cl">
						<label class="form-label col-xs-4 col-sm-3">新部门名称：</label>
...
<div id="editModal" class="modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content radius">
			<div class="modal-header">
				<h3 class="modal-title">修改部门名称</h3>
				<a class="close" data-dismiss="modal" aria-hidden="true" href="javascript:void();">×</a>
			</div>
			<div class="modal-body">
...
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页  <span class="c-gray en">&gt;</span> 部门管理 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
...
$(function(){
    ...
    //停、启用展示框
    $("body").on('click', 'a.dept-enable', function() {
    	var sData = api_table.fnGetData( $(this).parents("tr") );
    	if(sData.deptenable == 0){
    		//启用部门
    		layer.confirm('确定启用部门：'+sData.deptname+'？', {icon: 3, title:'提示'}, function(index){
    			changedeptstatus(1, sData.deptid);
        	});
    	}else if(sData.deptenable == 1){
    		//禁用部门
    		//获取部门列表
    		$.ajax({
   				type: 'POST',
   				...
   				success: function(data){
   					if(data.code == 200){
   						var opt = '<option value="">不分配</option>';
   						$.each(data.data, function(k, v) {
							if(sData.deptid != v.deptid){
								opt += '<option value="'+v.deptid+'">'+ v.deptname +'（已有'+ (v.count==null?0:v.count) +'人）</option>';
							}
						})
   						var str = 
   							'<div>将停用部门：<span class="label label-warning radius">'+ sData.deptname +'</span></div>' +
   							'部门：<span class="label label-warning  radius">'+ sData.deptname +'</span> 下的人员将移动至部门：' +
   							'<select class="select" name="deptid" size="1" style="width:auto;">' + opt +'</select>';
   						$("#changeModal .content-area").html(str);
   						$("#changeModal input[name='olddeptid']").val(sData.deptid);
	...
	//确定停用
	$("#changeModal .confirm").click(function() {
		changedeptstatus(0, $("#changeModal input[name='olddeptid']").val(), $("#changeModal select").val());
		$("#changeModal").modal('hide');
	})
	//部门停、启用
	var changedeptstatus = function(status, deptid, newdeptid) {
    	$.ajax({
			type: 'POST',
			url: 'editstatus',
			data: {status: status, deptid: deptid, tardeptid: newdeptid},
			success: function(data){
				if(data.code == 200){
