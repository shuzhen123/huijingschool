...
function Uploader(options) {
    ...
    var defaults = {
        ...
        //'JPG','PNG',
        fileExts: ['WMV','ASF','AVI','3GP','MKV','MP4','DVD','OGM','MOV','MPG','MPEG','MPE','FLV','F4V','SWF','M4V','QT','DAT','VOB','RMVB','RM','OGM','M2TS','MTS','TS','TP','WEBM','MP3','AAC'],
        /**
         * 获取初始化信息
        ...
        getInitInfo: function(file, callback) {
            ...
            $.ajax({
                ...
                headers: {
                    'AppKey': options.headers.AppKey,   //开发者平台分配的appkey
                    'Nonce': options.headers.Nonce,    //随机数（随机数，最大长度128个字符）
                    'CurTime': options.headers.CurTime,  //当前UTC时间戳，从1970年1月1日0点0分0秒开始到现在的秒数
                    'CheckSum': options.headers.CheckSum  //服务器认证需要,SHA1(AppSecret+Nonce+CurTime),16进制字符小写
                },
                dataType: 'json',
